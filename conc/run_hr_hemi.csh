...
# cd ../../..
 source /home/sinavo/pkg/cmaq/5.3/config_cmaq.csh $compiler $compilerVrsn
 cd /home/sinavo/pkg/cmaq/5.3/CCTM/da_9/con
 #cd /home/sinavo/pkg/cmaq/5.3/CCTM/scripts_con/con_1
 #cd $cwd
...
#> Set General Parameters for Configuring the Simulation
 ...
 set APPL      = hemi_h1_con         #> Application Name (e.g. Gridname)
 set ADDL      = HRNOW
 set ADDL0     = HRPAST
echo "ADDL=" $ADDL
...
#> is located by default).
 set BLD       = ${CMAQ_HOME}/CCTM/scripts_con/BLD_CCTM_${VRSN}_${compilerString}
...
#> Set Working, Input, and Output Directories
 setenv WORKDIR ${CMAQ_HOME}/CCTM/da_9/con       #> Working Directory. Where the runscript is.
 setenv OUTDIR  ${CMAQ_DATA}/cctm_hemi/hemi_h1/con9 #> Output Directory
 ...
 setenv LOGDIR  ${CMAQ_HOME}/CCTM/da_9/con     #> Log Directory Location
...
# setenv NEW_START TRUE             #> Set to FALSE for model restart
 ...
 set START_DATE = SYY-SMM-SDD     #> beginning date (Oct.  1, 2015)
 set END_DATE   = SYY-SMM-SDD     #> ending date    (Dec. 31, 2016)
 #set END_DATE   = EYY-EMM-EDD     #> ending date    (Dec. 32, 2016)

#> Set Timnestepping Parameters
 #set STTIME     = 000000           #> beginning GMT time (HHMMSS)
 set STTIME     = ${ADDL}           #> beginning GMT time (HHMMSS)
echo "STTIME=" $STTIME
 #set NSTEPS     = 240000           #> time duration (HHMMSS) for this run
 set NSTEPS     = 010000           #> time duration (HHMMSS) for this run
 set TSTEP      = 010000           #> output time step interval (HHMMSS)

  echo "sina $START_DATE"
 if ( $START_DATE == 2010-04-02 && $STTIME == 000000) then
...
# setenv NEW_START TRUE
echo "-dbg NEW_START =  $NEW_START"
# set START_DATE = "2010-04-02"     #> beginning date (Oct.  1, 2015)
# set END_DATE   = "2010-04-03"     #> ending date    (Dec. 31, 2016)
...
setenv GRIDDESC $INPDIR/mcip_hemi/mcip_h/GRIDDESC   #> grid description file
setenv GRID_NAME HEMI_108     #> check GRIDDESC file for GRID_NAME options
...
set ICpath    = $INPDIR/icon_hemi    #> initial conditions input directory 
#set BCpath    = $INPDIR/icbc/cb6r3_ae6    #> boundary conditions input directory
set BCpath    = $INPDIR/bcon_hemi    #> boundary conditions input directory
#set EMISpath  = $INPDIR/emis/cb6r3_ae6_fe #> gridded emissions input directory
set EMISpath  = $INPDIR/emis_hemi #> gridded emissions input directory
...
set METpath   = $INPDIR/mcip_hemi/mcip_h #> meteorology input directory 
...
set LUpath    = $INPDIR/emis_hemi  #> BELD landuse data for windblown dust model
#set SZpath    = /work/MOD3DEV/sgq/hemisphere/emission/cb05  #> surf zone file for in-line seaspray emissions
set SZpath    = $INPDIR/emis_hemi  #> surf zone file for in-line seaspray emissions
...
echo "I'M HERE!"
#set TODAYG = $START_DATE
echo "-dbg TODAYG = $TODAYG"
set TODAYJ = `date -ud "${START_DATE}" +%Y%j` #> Convert YYYY-MM-DD to YYYYJJJ
echo "-dbg TODAYJ = $TODAYJ"
set START_DAY = ${TODAYJ} 
set STOP_DAY = `date -ud "${END_DATE}" +%Y%j` #> Convert YYYY-MM-DD to YYYYJJJ
set NDAYS = 0 

while ($TODAYJ <= $STOP_DAY )  #>Compare dates in terms of YYYYJJJ
echo "while ($TODAYJ <= $STOP_DAY )"  
...
# =====================================================================
  ...
  setenv CTM_APPL0 ${RUNID}_${YESTERDAY} 
echo "CTM_APPL=" $CTM_APPL
  setenv CTM_TIME ${ADDL} 
  setenv CTM_TIME0 ${ADDL0}
 echo "CTM_TIME=" $CTM_TIME
  #> Copy Model Configuration To Output Folder
  if ( ! -d "$OUTDIR" ) mkdir -p $OUTDIR
  cp $BLD/CCTM_${VRSN}.cfg $OUTDIR/CCTM_${CTM_APPL}_${CTM_TIME}.cfg
...
echo "if ($NEW_START == true || $NEW_START == TRUE )"
     #setenv ICFILE CCTM_CGRID_v52_hdifupdate_intel17.0_HEMIS_cb6_20150930
     #setenv ICFILE ICON_CH4_VAR_v53_HEMI_regrid_20100402
     #setenv ICFILE ICON_CH4_CON_NASA_HEMI_regrid_20100409_w1
     setenv ICFILE ICON_CH4_CON_NASA_HEMI_biascorrected_20100402
    # setenv ICFILE ICON_v53_HEMI_regrid_20100413  ICON_CH4_CON_NASA_HEMI_biascorrected_20100402
     setenv INIT_MEDC_1 notused
     setenv INITIAL_RUN Y #related to restart soil information file
  else if ( $START_DATE != 2010-04-02 && $STTIME == 000000 ) then
echo " if ( $START_DATE != 2010-04-02 && $STTIME == 000000 )"
     set ICpath = $OUTDIR
     setenv ICFILE CCTM_CGRID_${CTM_APPL0}_230000.nc
     setenv INIT_MEDC_1 $ICpath/CCTM_MEDIA_CONC_${CTM_APPL0}
     setenv INIT_MEDC_1 notused
     setenv INITIAL_RUN N #N
  else
     set ICpath = $OUTDIR
     setenv ICFILE CCTM_CGRID_${CTM_APPL}_${CTM_TIME0}.nc
     setenv INIT_MEDC_1 $ICpath/CCTM_MEDIA_CONC_${CTM_APPL}
     setenv INIT_MEDC_1 notused
     setenv INITIAL_RUN N #N
  endif
  ...
  #set BCFILE = BCON_v53_HEMI_regrid_20100402
  #set BCFILE = BCON_CH4_v53_HEMI_regrid_20100413
  set BCFILE = BCON_CH4_CON_v53_HEMI_regrid_${YYYYMMDD}
  #set BCFILE = BCON_CH4_TRACER_v53_HEMI_regrid_20100402
  ...
  set MCONF = HEMI_108
  setenv GRID_BDY_2D $METpath/GRIDBDY2D_${MCONF}_${YYYYMMDD}.nc # GRID files are static, not day-specific
  setenv GRID_CRO_2D $METpath/GRIDCRO2D_${MCONF}_${YYYYMMDD}.nc
  setenv GRID_CRO_3D absent
  setenv GRID_DOT_2D $METpath/GRIDDOT2D_${MCONF}_${YYYYMMDD}.nc
  setenv MET_CRO_2D   $METpath/METCRO2D_${MCONF}_${YYYYMMDD}.nc
  setenv MET_CRO_3D   $METpath/METCRO3D_${MCONF}_${YYYYMMDD}.nc
  setenv MET_DOT_3D   $METpath/METDOT3D_${MCONF}_${YYYYMMDD}.nc
  setenv MET_BDY_3D   $METpath/METBDY3D_${MCONF}_${YYYYMMDD}.nc
  setenv LUFRAC_CRO   $METpath/LUFRAC_CRO_${MCONF}_${YYYYMMDD}.nc
...
#> Spatial Masks For Emissions Scaling
  ...
  if( $YEAR == 2015 ) then
      ...
      set EMISfile  = EMIS_CH4_TRACER_HEMI_${YYYYMMDD}.nc 
      #set EMISfile  = EMIS_CH4_TRACER_HEMI_20100413.nc 
  endif
 ...
 ### added by sina
# setenv N_EMIS_TR 1
 setenv TR_EMIS_01 ${EMISpath}/${EMISfile}
 setenv TR_EMIS_LAB_01 GRIDDED_EMIS
 setenv EMIS_TRAC_1 $TR_EMIS_01
 ####
...
#> Integrated Source Apportionment Method (ISAM) Options
 ...
 if ( $?CTM_ISAM ) then
    if ( $CTM_ISAM == 'Y' || $CTM_ISAM == 'T' ) then
       ...
       setenv SA_ACONC_1      "$OUTDIR/CCTM_SA_ACONC_${CTM_APPL}_${CTM_TIME}.nc -v"
       setenv SA_CONC_1       "$OUTDIR/CCTM_SA_CONC_${CTM_APPL}_${CTM_TIME}.nc -v"
       setenv SA_DD_1         "$OUTDIR/CCTM_SA_DRYDEP_${CTM_APPL}_${CTM_TIME}.nc -v"
       setenv SA_WD_1         "$OUTDIR/CCTM_SA_WETDEP_${CTM_APPL}_${CTM_TIME}.nc -v"
       setenv SA_CGRID_1      "$OUTDIR/CCTM_SA_CGRID_${CTM_APPL}_${CTM_TIME}.nc -v"
...
# =====================================================================
  ...
  setenv S_CGRID         "$OUTDIR/CCTM_CGRID_${CTM_APPL}_${CTM_TIME}.nc"         #> 3D Inst. Concentrations
  setenv CTM_CONC_1      "$OUTDIR/CCTM_CONC_${CTM_APPL}_${CTM_TIME}.nc -v"       #> On-Hour Concentrations
  setenv A_CONC_1        "$OUTDIR/CCTM_ACONC_${CTM_APPL}_${CTM_TIME}.nc -v"      #> Hourly Avg. Concentrations
  setenv MEDIA_CONC      "$OUTDIR/CCTM_MEDIA_CONC_${CTM_APPL}_${CTM_TIME}.nc -v" #> NH3 Conc. in Media
  setenv CTM_DRY_DEP_1   "$OUTDIR/CCTM_DRYDEP_${CTM_APPL}_${CTM_TIME}.nc -v"     #> Hourly Dry Deposition
  setenv CTM_DEPV_DIAG   "$OUTDIR/CCTM_DEPV_${CTM_APPL}_${CTM_TIME}.nc -v"       #> Dry Deposition Velocities
  setenv B3GTS_S         "$OUTDIR/CCTM_B3GTS_S_${CTM_APPL}_${CTM_TIME}.nc -v"    #> Biogenic Emissions
  setenv SOILOUT         "$OUTDIR/CCTM_SOILOUT_${CTM_APPL}_${CTM_TIME}.nc"       #> Soil Emissions
  setenv CTM_WET_DEP_1   "$OUTDIR/CCTM_WETDEP1_${CTM_APPL}_${CTM_TIME}.nc -v"    #> Wet Dep From All Clouds
  setenv CTM_WET_DEP_2   "$OUTDIR/CCTM_WETDEP2_${CTM_APPL}_${CTM_TIME}.nc -v"    #> Wet Dep From SubGrid Clouds
  setenv CTM_PMDIAG_1    "$OUTDIR/CCTM_PMDIAG_${CTM_APPL}_${CTM_TIME}.nc -v"     #> On-Hour Particle Diagnostics
  setenv CTM_APMDIAG_1   "$OUTDIR/CCTM_APMDIAG_${CTM_APPL}_${CTM_TIME}.nc -v"    #> Hourly Avg. Particle Diagnostics
  setenv CTM_RJ_1        "$OUTDIR/CCTM_PHOTDIAG1_${CTM_APPL}_${CTM_TIME}.nc -v"  #> 2D Surface Summary from Inline Photolysis
  setenv CTM_RJ_2        "$OUTDIR/CCTM_PHOTDIAG2_${CTM_APPL}_${CTM_TIME}.nc -v"  #> 3D Photolysis Rates 
  setenv CTM_RJ_3        "$OUTDIR/CCTM_PHOTDIAG3_${CTM_APPL}_${CTM_TIME}.nc -v"  #> 3D Optical and Radiative Results from Photolysis
  setenv CTM_SSEMIS_1    "$OUTDIR/CCTM_SSEMIS_${CTM_APPL}_${CTM_TIME}.nc -v"     #> Sea Spray Emissions
  setenv CTM_DUST_EMIS_1 "$OUTDIR/CCTM_DUSTEMIS_${CTM_APPL}_${CTM_TIME}.nc -v"   #> Dust Emissions
  setenv CTM_IPR_1       "$OUTDIR/CCTM_PA_1_${CTM_APPL}_${CTM_TIME}.nc -v"       #> Process Analysis
  setenv CTM_IPR_2       "$OUTDIR/CCTM_PA_2_${CTM_APPL}_${CTM_TIME}.nc -v"       #> Process Analysis
  setenv CTM_IPR_3       "$OUTDIR/CCTM_PA_3_${CTM_APPL}_${CTM_TIME}.nc -v"       #> Process Analysis
  setenv CTM_IRR_1       "$OUTDIR/CCTM_IRR_1_${CTM_APPL}_${CTM_TIME}.nc -v"      #> Chem Process Analysis
  setenv CTM_IRR_2       "$OUTDIR/CCTM_IRR_2_${CTM_APPL}_${CTM_TIME}.nc -v"      #> Chem Process Analysis
  setenv CTM_IRR_3       "$OUTDIR/CCTM_IRR_3_${CTM_APPL}_${CTM_TIME}.nc -v"      #> Chem Process Analysis
  setenv CTM_DRY_DEP_MOS "$OUTDIR/CCTM_DDMOS_${CTM_APPL}_${CTM_TIME}.nc -v"      #> Dry Dep
  setenv CTM_DRY_DEP_FST "$OUTDIR/CCTM_DDFST_${CTM_APPL}_${CTM_TIME}.nc -v"      #> Dry Dep
  setenv CTM_DEPV_MOS    "$OUTDIR/CCTM_DEPVMOS_${CTM_APPL}_${CTM_TIME}.nc -v"    #> Dry Dep Velocity
  setenv CTM_DEPV_FST    "$OUTDIR/CCTM_DEPVFST_${CTM_APPL}_${CTM_TIME}.nc -v"    #> Dry Dep Velocity
  setenv CTM_VDIFF_DIAG  "$OUTDIR/CCTM_VDIFF_DIAG_${CTM_APPL}_${CTM_TIME}.nc -v" #> Vertical Dispersion Diagnostic
  setenv CTM_VSED_DIAG   "$OUTDIR/CCTM_VSED_DIAG_${CTM_APPL}_${CTM_TIME}.nc -v"  #> Particle Grav. Settling Velocity
  setenv CTM_LTNGDIAG_1  "$OUTDIR/CCTM_LTNGHRLY_${CTM_APPL}_${CTM_TIME}.nc -v"   #> Hourly Avg Lightning NO
  setenv CTM_LTNGDIAG_2  "$OUTDIR/CCTM_LTNGCOL_${CTM_APPL}_${CTM_TIME}.nc -v"    #> Column Total Lightning NO
  setenv CTM_VEXT_1      "$OUTDIR/CCTM_VEXT_${CTM_APPL}_${CTM_TIME}.nc -v"       #> On-Hour 3D Concs at select sites

  #> set floor file (neg concs)
  setenv FLOOR_FILE ${OUTDIR}/FLOOR_${CTM_APPL}_${CTM_TIME}.txt

  #> look for existing log files and output files
  #( ls CTM_LOG_???.${CTM_APPL}_${CTM_TIME} > buff.txt ) >& /dev/null
  ( ls CTM_LOG_???.${CTM_APPL} > buff.txt ) >& /dev/null
  ( ls ${LOGDIR}/CTM_LOG_???.${CTM_APPL} >> buff.txt ) >& /dev/null
 # ( ls ${LOGDIR}/CTM_LOG_???.${CTM_APPL}_${CTM_TIME} >> buff.txt ) >& /dev/null
...
# ===================================================================
  ...
  #mv CTM_LOG_???.${CTM_APPL}_${CTM_TIME} $LOGDIR
  mv CTM_LOG_???.${CTM_APPL} $LOGDIR
  if ( $CTM_DIAG_LVL != 0 ) then
    #mv CTM_DIAG_???.${CTM_APPL}_${CTM_TIME} $LOGDIR
